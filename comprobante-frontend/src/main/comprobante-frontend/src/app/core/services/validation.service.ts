...
export class ValidationService {
  ...
  uppercaseFormValues(group: FormGroup, exclusions?: string[]): void {
    Object.keys(group.controls).forEach((key: string) => {
      let abstractControl = group.get(key);
      if (abstractControl instanceof FormGroup) {
        this.uppercaseFormValues(abstractControl, exclusions);
      } else {
        if (typeof exclusions != 'undefined') {
          let ex = exclusions.find(el => el == key);
          if (!ex) {
            abstractControl.setValue(abstractControl.value.toUpperCase());
          }
        } else {
          abstractControl.setValue(abstractControl.value.toUpperCase());
        }
